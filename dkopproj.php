...
<?php
     function ShowTable($table_name) {
		...
		if( isset($_GET['status']) ){
			$sql="select a.ip,a.cid,a.cname,a.image,a.createtime,a.status, b.projname,b.progdir,b.startscript,b.stopscript,d.pid from dockercontainers as a inner join dkcreated as c on c.cname=a.cname inner join dockerproject as b on c.project=b.projname left join dkprojstatus as d on d.cid=a.cid where status=''";
		}
        ...
        for($i=0; $i < $colums; $i++){
			...
			if (!empty($_GET['projname'])){ 
				echo "<th><a href='dkopproj.php?sortby=".$field_name."&projname=".$_GET['projname']."'>".$field_name."</a></th>\n";			//  target='right' 
			}else{
        ...
        for($i=0; $i<count($dkstatusres); $i++){
			...
			if( !empty($_REQUEST[$tmp]) ){
				...
				for ($j=0; $j<count($hostip); $j++) {
					if( $hostip[$j]==$j."-".$dkstatusres[$i][ip] ){
						$selhostname=$hostip[$j];
					}
					//echo $hostip[$j]."<br>";
				}
				for ($j=0; $j<count($projnames); $j++) {
					if( $projnames[$j]==$j."-".$dkstatusres[$i]['cname'] ){
						$selprojname=$projnames[$j];
					}
					//echo $projnames[$j]."<br>";
				}
				$url="tailfdown.php?tailf=1&lines=50&keyword=".$dkstatusres[$i]['projname']."&selhostname=".$selhostname."&selprojname=".$selprojname;
				//echo $url;
				header('Location:'.$url);
